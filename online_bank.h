...
void withdrawal_record(int user_index ,unsigned int amount);
void withdrawal_money(int user_index, unsigned int amount);
...
void userSector(){
    ...
    if(user_option == 1){
        ...
        if( Input_amount <= db[emailExist].current_amount){
            ...
            withdrawal_money(emailExist,Input_amount);
            userSector();
        } else{
...
//            for(int dcc=0; dcc<space_arrary[ncc]-19 ; dcc++){
//                fprintf(fptr,"%s",&db[ncc].tr_record[dcc].withdrawal_rc);
//            }
...
void printingAllData(){
    ...
    for(int ncc=0; ncc<users ; ncc++){
        ...
        for(int dcc=0; dcc<space_arrary[ncc]-19 ; dcc++){
            printf("-%s",&db[ncc].tr_record[dcc].withdrawal_rc[0]);
        }
        printf("\n");
    }
}
...
void recording_alldata_toFile(){
    ...
    for(int ncc=0; ncc<users; ncc++){
        ...
        for(int dcc=0; dcc<space_arrary[ncc]-19 ; dcc++){
            fprintf(fptr, "%s", &db[ncc].tr_record[dcc].withdrawal_rc[0]);
        }
        fprintf(fptr,"%c",'\n');
    }
}
...
// Winhtut-withdraw$300
    int user_name_counter = charCounting(db[user_index].name);
...
//    int amount_counter = charCounting(int_to_char_array_data);
    ...
    char withdraw[10]={'-','w','i','t','h','d','r','a','w','-'};


    int index_point=0;
    for(int i=0; i<user_name_counter; i++){
        db[user_index].tr_record[space_arrary[user_index]-19].withdrawal_rc[i] = db[user_index].name[i];
        index_point++;
    }

    for(int a=0; a<10; a++){

        db[user_index].tr_record[space_arrary[user_index]-19].withdrawal_rc[index_point]= withdraw[a];
        index_point++;

    }
    for(int aa=0; aa<amount_counter; aa++){

        db[user_index].tr_record[space_arrary[user_index]-19].withdrawal_rc[index_point]= int_to_char_arrayData[aa];
        index_point++;
    }


//    printf("withdrawal record: %s", db[user_index].tr_record[user_index]withdrawal_rc);
    space_arrary[user_index] +=1;

}

void withdrawal_money(int user_index,unsigned int amount){

    printf("loading to withdraw...\n");

    // insert withdrawal limit function

    db[user_index].current_amount -= amount;
    printf("your withdrawal is successful!!\n");
    withdrawal_record(user_index, amount);
